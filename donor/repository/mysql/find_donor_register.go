...
func (m *mysqlDonorRepository) fetchDonorRegister(ctx context.Context, query string, args ...interface{}) (result []*domain.DonorRegisterDTO, err error) {
...
func (m *mysqlDonorRepository) FindDonorRegister(ctx context.Context, id int64) ([]*domain.DonorRegisterDTO, error) {
	...
	WHERE donor_schedulle.id = ? 
	ORDER BY donor_registers.createdAt`

	res, err := m.fetchDonorRegister(ctx, query, id)
	if err != nil {
		return nil, err
	}
