...
func (m *mysqlDonorRepository) LastDonorByStatus(ctx context.Context, userId int64, status string) (res *domain.DonorRegisterDTO, err error) {
	...
	WHERE userId = ? 
	AND status = ? 
	ORDER BY donor_schedulle.date DESC`
