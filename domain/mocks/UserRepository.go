...
import (
	...
	model "github.com/bxcodec/go-clean-arch/user/repository/model"
)
...
func (_m *UserRepository) GetProfileFull(ctx context.Context, userId int64) (*model.UserModel, error) {
	ret := _m.Called(ctx, userId)

	var r0 *model.UserModel
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, int64) (*model.UserModel, error)); ok {
		return rf(ctx, userId)
	}
	if rf, ok := ret.Get(0).(func(context.Context, int64) *model.UserModel); ok {
		r0 = rf(ctx, userId)
	} else {
		if ret.Get(0) != nil {
